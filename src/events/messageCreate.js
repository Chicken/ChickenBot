module.exports = async (client, message) => {
        ...
        if (settings.xp) {
            ...
            if (!client.cooldown.has(message.author.id)) {
                ...
                const level = Math.floor(
                    0.1 *
                        Math.sqrt(client.db.get(message.guild.id, `users.${message.author.id}.xp`))
                );
                const oldlevel = client.db.get(
                    message.guild.id,
    ...
    const pingRegExp = new RegExp(`^<@!?${client.user.id}>$`, "m");
    ...
    const perm = await client.perm(message);
    const cmd = client.commands.get(command) || client.commands.get(client.aliases.get(command));
    if (!cmd) return;
    if (cmd.data.disabled)
        return message.channel.send(
            "Hey! Sorry but that command is currently disabled (Probably broken!) Spam ping the dev lmao! :D"
        );
    ...
    if (
        message.guild &&
        !message.channel.permissionsFor(message.guild.me).has(cmd.data.permissions, true)
    ) {
        let txt =
            "Hey! I am missing some permissions to run this command!\nMissing permissions listed below!\n";
        message.channel
            .permissionsFor(message.guild.me)
            .missing(cmd.data.permissions, true)
            .forEach((p) => {
    ...
    try {
        ...
        client.logger.error(`Error running ${command}`);
        // I use util.inspect here so i get the stack trace
        // cause chicken's logger doesn't do that.
        client.logger.error(require("util").inspect(error));
        message.channel.send(`I'm sorry an error occurred while running that command!
\`\`\`
${error}
\`\`\``);
    }
