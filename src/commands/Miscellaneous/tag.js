...
exports.execute = async (client, message, args) => {
    ...
    const tags = client.db.get(message.guild.id, "tags");
    ...
    switch (args[0]) {
        case "list": {
            ...
            message.channel.send(
                `Tags on this server (\`${Object.keys(tags).length}\` total): ${taglist}`
            );
            break;
        }
        case "add": {
            ...
            client.db.set(
                message.guild.id,
                {
                    content: message.cleanContent.split(" ").slice(3).join(" "),
        ...
        default: {
            ...
            const opt = {
                content: (tag.content || "") + (tag.attachments.length ? "\n" + tag.attachments.join(" ") : ""),
            };
