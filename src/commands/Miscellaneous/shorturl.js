...
exports.execute = async (client, message, args) => {
    if(args[0] == "info") {
        ...
        if(!/^https:\/\/antti\.link\/[a-z0-9]+\/?$/im.test(url)) return message.channel.send("Please provide a valid antti.link url!");
        try {
            let {
                status,
                id,
        ...
        let full = encodeURI(args.join(" "));
        if(!full) return message.channel.send("Please provide a valid url!");

        let url;
        try {
            url = (await bent("POST", 200, 201, "json", "https://antti.link/new")(null, {
                url: full
            })).url;
        } catch (e) {
            return message.channel.send("Error occured! Please try again.");
