...
exports.execute = async (client, message, args) => {
    ...
    if (
        fs.existsSync(
            `./ytdl/${audioOnly ? "audio" : "video"}/${info.player_response.videoDetails.videoId}${
                audioOnly ? ".mp3" : ".mp4"
            }`
        )
    ) {
        return message.reply(
            `Done! Here's a link you can download from! (expires someday in the future) <${
    ...
    if (audioOnly) {
        ...
        ffmpeg(stream)
            ...
            .on("end", () =>
                message.reply(
                    `Done! Here's a link you can download from! (expires someday in the future) <${client.config.hostname}/ytdl/audio/${info.player_response.videoDetails.videoId}>`
                )
            );
    } else {
        ytdl.downloadFromInfo(info, {
            ...
            .pipe(
                fs.createWriteStream(
                    `./ytdl/video/${info.player_response.videoDetails.videoId}.mp4`
                )
            )
            .on("close", () =>
                message.reply(
                    `Done! Here's a link you can download from! (expires someday in the future) <${client.config.hostname}/ytdl/video/${info.player_response.videoDetails.videoId}>`
                )
            );
    }
};
