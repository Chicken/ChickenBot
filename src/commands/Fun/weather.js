...
exports.execute = async (client, message, args) => {
    if (!args[0]) {
        ...
        const apikey = client.config.weatherapi;
        ...
        const weatherEmbed = new MessageEmbed()
            ...
            .addField(
                ":dash: Wind speed and direction:",
                `${weather.wind.speed}m/s, ${getDirectionText(weather.wind.deg)}`
            )
            .addField(
                "Country:",
                `:flag_${weather.sys.country.toLowerCase()}: ${weather.sys.country}`
            )
