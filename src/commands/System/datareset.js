...
exports.execute = async (client, message, args) => {
    if (!args[0]) {
        return message.channel.send(
            "Do you want to get your user data reset or remove the guild from the database. Usage is `datareset <user | guild>`"
        );
    }
    switch (args[0]) {
        case "user":
            message.channel.send(
                "Removing your data... (this does not apply to leveling system xp, this data will be removed upon leaving the guilds using this system)"
            );
        ...
        case "guild":
            ...
            if ((await client.perm(message)) < 3) {
