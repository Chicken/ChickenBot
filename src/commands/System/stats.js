...
exports.execute = async (client, message, args) => {
    const embed = new Discord.MessageEmbed()
        ...
        .addField("⏱Uptime", client.formatDuration(client.uptime) || "0 sec", true)
        .addField("💽Ram", `${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)} MB`, true)
        .addField(
            `${client.ws.ping > 100 ? "<a:e:525001588935360522>" : "<a:e:524998745721536514>"}Ping`,
            `${Math.floor(client.ws.ping)}ms!`,
            true
        )
        .addField(
            "👥Users",
            `${client.users.cache.size} users, in ${client.channels.cache.size} channels of ${client.guilds.cache.size} servers.`,
            true
        )
